...
y_rod_mount_left(true);

module y_rod_mount_left(print_orientation = false){
  LX = y_rod_mount_LX;
  LY = y_rod_mount_LY;
  LZ = y_rod_mount_LZ;
  ...
  module body(){
    difference(){
      ...
      // roller bearing hole
      translate([LX - roll_T - 0.1, LY/2, ply_LZ + interior_z_padding + nema_W/2])
      rotate([0, 90, 0])
      cylinder(d = roll_OD + roll_fit, h = roll_T + 0.2);
      
      // roller bearing chamfer
      translate([LX, LY/2, ply_LZ + interior_z_padding + nema_W/2])
      rotate([0, 90, 0])
      chamfer_cut(roll_OD + roll_fit, y_hole_chamfer);
